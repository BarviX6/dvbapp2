...
class HdmiCECSetupScreen(Screen, ConfigListScreen):
    ...
    def advancedSettings(self, mode = None, savevalues = False):
        advconfigs = ('deepstandby_waitfortimesync', 'tv_standby_notinputactive', 'tv_wakeup_zaptimer', 'tv_wakeup_zapandrecordtimer', 'tv_wakeup_wakeuppowertimer', 'workaround_activesource', 'handle_tv_input', 'handle_tv_delaytime', 'messages_repeat', 'check_tv_state', 'workaround_turnbackon')
    ...
    def showHelpText(self):
        ...
        elif cur and len(cur) > 3 and cur[3] not in ('refreshlist', _('helptext')):
