...
class MultiBootSelector(Screen, HelpableScreen):
    ...
    def __init__(self, session, *args):
        ...
        self['actions'] = HelpableActionMap(self, ['OkCancelActions',
         ...
         'MenuActions'], {'red': (boundFunction(self.close, None), _('Cancel the image selection and exit')),
         ...
         'menu': (boundFunction(self.close, True), _('Cancel the image selection and exit all menus'))}, -1, description=_('MultiBootSelector Actions'))
        imagedict = []
            ...
            for index, x in enumerate(sorted(imagedict.keys())):
                if imagedict[x]['imagename'] != _('Empty slot'):
                    if SystemInfo['canMode12']:
                        list.insert(index, ChoiceEntryComponent('', (slotMulti % (x,
                          imagedict[x]['imagename'],
                          1,
                          current if x == currentimageslot and mode != 12 else ''), x)))
                        list.append(ChoiceEntryComponent('', (slotMulti % (x,
                          imagedict[x]['imagename'],
                          12,
                          current if x == currentimageslot and mode == 12 else ''), x + 12)))
                        indextot = index + 1
                    else:
