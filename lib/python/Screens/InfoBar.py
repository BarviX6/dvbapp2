...
class InfoBar(InfoBarBase, InfoBarShowHide,
	InfoBarNumberZap, InfoBarChannelSelection, InfoBarMenu, InfoBarEPG, InfoBarRdsDecoder, InfoBarVmodeButton, HelpableScreen,
	InfoBarInstantRecord, InfoBarAudioSelection, InfoBarRedButton, InfoBarTimerButton, InfoBarResolutionSelection, InfoBarAspectSelection, InfoBarAdditionalInfo,
	InfoBarNotifications, InfoBarDish, InfoBarUnhandledKey, InfoBarLongKeyDetection, InfoBarSubserviceSelection, InfoBarTimeshift, InfoBarSeek, InfoBarCueSheetSupport, InfoBarBuffer,
	InfoBarSummarySupport, InfoBarTimeshiftState, InfoBarTeletextPlugin, InfoBarExtensions, InfoBarPiP, InfoBarPlugins, InfoBarSubtitleSupport, InfoBarServiceErrorPopupSupport, InfoBarJobman,
	InfoBarZoom, InfoBarSleepTimer, InfoBarOpenOnTopHelper, InfoBarHandleBsod, InfoBarHdmi, Screen):
    ALLOW_SUSPEND = True
    instance = None

    def __init__(self, session):
        Screen.__init__(self, session)
        self['actions'] = HelpableActionMap(self, 'InfobarActions', {'showMovies': (self.showMovies, _('Play recorded movies...')),
         ...
         'openSimpleUnmount': (self.openSimpleUnmount, _('Simple umounter mass storage device.'))}, prio=2)
        self['key_red'] = Label()
...
class MoviePlayer(InfoBarAspectSelection, InfoBarSimpleEventView, InfoBarBase, InfoBarShowHide, InfoBarLongKeyDetection, InfoBarMenu, InfoBarEPG, InfoBarSeek, InfoBarShowMovies, InfoBarInstantRecord, InfoBarAudioSelection, InfoBarResolutionSelection, HelpableScreen, InfoBarNotifications, InfoBarServiceNotifications, InfoBarPVRState, InfoBarCueSheetSupport, InfoBarMoviePlayerSummarySupport, InfoBarSubtitleSupport, Screen, InfoBarTeletextPlugin, InfoBarServiceErrorPopupSupport, InfoBarExtensions, InfoBarPlugins, InfoBarPiP, InfoBarZoom, InfoBarHdmi):
    ...
    def __init__(self, session, service, slist = None, lastservice = None, infobar = None):
        ...
        self['actions'] = HelpableActionMap(self, 'MoviePlayerActions', {'InfoButtonPressed': (self.openEventView, _('open Info...')),
        ...
        self['DirectionActions'] = HelpableActionMap(self, 'DirectionActions', {'left': self.left,
         'right': self.right}, prio=-2)
        self.allowPiP = True
        for x in (InfoBarAspectSelection,
         HelpableScreen,
         InfoBarShowHide,
         InfoBarLongKeyDetection,
         InfoBarMenu,
            ...
            if slist.inBouquet():
                ...
                if prev:
                    ...
                    while True:
                        ...
                        cur = slist.getCurrentSelection()
                        if cur:
                            playable = not cur.flags & 72 and hasattr(self.session, 'pip') and self.session.pip.isPlayableForPipService(cur)
