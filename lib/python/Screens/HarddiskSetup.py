...
class HarddiskSetup(Screen):
    ...
    def __init__(self, session, hdd, action, text, question):
        ...
        self['actions'] = ActionMap(['OkCancelActions'], {'ok': self.hddQuestion,
         'cancel': self.close})
        self['shortcuts'] = ActionMap(['ShortcutActions'], {'red': self.hddQuestion})
...
class HarddiskConvertExt4Selection(HarddiskSelection):
    ...
    def doIt(self, selection):
        self.session.openWithCallback(self.close, HarddiskSetup, selection, action=selection.createExt4ConversionJob, text=_('Convert ext3 to ext4'), question=_('Do you really want to convert the filesystem?\nYou cannot go back!'))
