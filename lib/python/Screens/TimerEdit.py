...
class TimerEditList(Screen):
    ...
    def toggleDisabledState(self):
        ...
        if cur:
            ...
            if t.disabled:
                ...
                if t.repeated:
                    list = ((_('Stop current event but not coming events'), 'stoponlycurrent'), (_('Stop current event and disable coming events'), 'stopall'), (_("Don't stop current event but disable coming events"), 'stoponlycoming'))
    ...
    def getEPGEvent(self, timer):
        ...
        if timer:
            ...
            if not event:
                ...
                if duration <= 0:
                    duration = 30
                list = epgcache.lookupEvent(['IBDT', (ref,
                  0,
                  begin,
                  duration)])
                if len(list):
                    ...
                    if not event_id and timer.begin != timer.end:
                        for epgevent in list:
    ...
    def removeTimerQuestion(self):
        ...
        if onhdd:
            ...
            choices = [(_('No'), 'no'), (_('Yes, delete from Timerlist'), 'yes'), (_('Yes, delete from Timerlist and delete recording'), 'yesremove')]
...
class TimerSanityConflict(Screen):
    ...
    def toggleTimer(self):
        x = self['list'].getSelectedIndex() + 1
