...
class ScanSetup(ConfigListScreen, Screen, CableTransponderSearchSupport, TerrestrialTransponderSearchSupport):
    ...
    def newConfig(self):
        ...
        if cur is not None:
            ...
            if cur in (self.typeOfScanEntry,
             ...
             self.modulationEntry):
                ...
                if cur[1] in (self.scan_ter.bandwidth,
    ...
    def createConfig(self):
        ...
        for n in nimmanager.nim_slots:
            ...
            if dvbc or dvbt or atsc:
                ...
                if n.canBeCompatible('DVB-S') and len(nimmanager.getSatListForNim(n.slot)) < 1:
                    continue
    ...
    def keyGoCheckTimeshiftCallback(self, answer):
        ...
        if nim.isCompatible('DVB-S'):
            ...
            if 'multisat' in self.scan_type.value:
                ...
                if len(nimsats):
                    ...
                    if self.scan_type.value == 'single_transponder':
                        ...
                        self.addSatTransponder(tlist, self.scan_sat.frequency.value, self.scan_sat.symbolrate.value, self.scan_sat.polarization.value, fec, self.scan_sat.inversion.value, orbpos, self.scan_sat.system.value, self.scan_sat.modulation.value, self.scan_sat.rolloff.value, self.scan_sat.pilot.value, self.scan_sat.is_id.value, self.scan_sat.pls_mode.value, self.scan_sat.pls_code.value, self.scan_sat.t2mi_plp_id.value, self.scan_sat.t2mi_pid.value)
            ...
            if self.scan_typecable.value == 'single_transponder':
                self.addCabTransponder(tlist, self.scan_cab.frequency.floatint, self.scan_cab.symbolrate.value * 1000, self.scan_cab.modulation.value, self.scan_cab.fec.value, self.scan_cab.inversion.value)
                removeAll = False
            elif self.scan_typecable.value == 'predefined_transponder':
                ...
                if config.Nims[index_to_scan].dvbc.scan_type.value == 'provider':
                    ...
                    self.addCabTransponder(tlist, 73000, 793000 / 1000, eDVBFrontendParametersCable.Modulation_Auto, eDVBFrontendParametersCable.FEC_Auto, eDVBFrontendParametersCable.Inversion_Unknown)
                    removeAll = False
                else:
            ...
            if self.scan_typeatsc.value == 'single_transponder':
                self.addATSCTransponder(tlist, frequency=self.scan_ats.frequency.floatint * 1000, modulation=self.scan_ats.modulation.value, inversion=self.scan_ats.inversion.value, system=self.scan_ats.system.value)
                removeAll = False
            elif self.scan_typeatsc.value == 'predefined_transponder':
    ...
    def humanReadableCabTransponder(self, tp):
        if tp[3] in range(7) and (tp[4] in range(10) or tp[4] == 15):
            ...
            fec_list = {0: 'Auto',
             ...
             9: '9/10',
             15: 'None'}
            return str(tp[1] / 1000.0) + ' MHz ' + fec_list[tp[4]] + ' ' + str(tp[2] / 1000) + ' ' + mod_list[tp[3]]
        return _('Invalid transponder data')
