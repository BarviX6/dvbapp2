...
class VideoSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['SetupActions', 'MenuActions', 'ColorActions'], {'cancel': self.keyCancel,
         'save': self.apply,
    ...
    def createSetup(self):
        ...
        self.list = [getConfigListEntry(_('Video output'), config.av.videoport, _('Configures which video output connector will be used.'))]
        ...
        if force_wide or config.av.aspect.value in ('16:9', '16:10'):
            self.list.extend((getConfigListEntry(_('Display 4:3 content as'), config.av.policy_43, _('When the content has an aspect ratio of 4:3, choose whether to scale/stretch the picture.')), getConfigListEntry(_('Display >16:9 content as'), config.av.policy_169, _('When the content has an aspect ratio of 16:9, choose whether to scale/stretch the picture.'))))
    ...
    def confirm(self, confirmed):
        if not confirmed:
            if self.reset_mode == 1:
                ...
                try:
                    if self.current_mode not in iAVSwitch.readAvailableModes():
    ...
    def apply(self):
        ...
        if config.av.autores.value in ('all', 'hd') and ((port, mode, rate) != self.last_good or (autores_sd, smart1080p) != self.last_good_extra):
            ...
            elif smart1080p == '1080p50' or smart1080p == 'true':
                self.hw.setMode(port, '1080p', '50Hz')
            ...
            self.reset_mode = 1
            self.hw.setMode(port, mode, rate)
        elif config.av.autores.value in ('native', 'simple') and ((port, mode_sd, rate_sd) != self.last_good_autores_sd or (port, mode_hd, rate_hd) != self.last_good_autores_hd or (port, mode_fhd, rate_fhd) != self.last_good_autores_fhd or (port, mode_uhd, rate_uhd) != self.last_good_autores_uhd or (autores_24p, autores_1080i) != self.last_good_autores_extra or self.last_good_autores != config.av.autores.value or self.reset_mode == 1 or self.last_good_autores_unknownres != config.av.autores_unknownres.value and config.av.autores.value == 'native'):
            self.reset_mode = 2
...
class AutoVideoMode(Screen):
    ...
    def VideoChangeDetect(self):
        ...
        print '[VideoMode] detect video height: %s, width: %s, pol: %s, rate: %s (current video mode: %s)' % (video_height,
         video_width,
         video_pol,
         video_rate,
         current_mode)
...
def autostart(session):
    global resolutionlabel
    if not getAutoresPlugin_enabled():
