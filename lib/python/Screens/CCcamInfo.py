...
TranslationHelper = [['Current time', _('Current time')],
 ['NodeID', _('NodeID')],
 ['Uptime', _('Uptime')],
...
def CCcamListEntry(name, idx):
    ...
    if fileExists(png):
        x, y, w, h = skin.parameters.get('ChoicelistIcon', (5 * f,
         0,
         35 * f,
         25 * f))
        res.append(MultiContentEntryPixmapAlphaBlend(pos=(x, y), size=(w, h), png=loadPNG(png)))
    x, y, w, h = skin.parameters.get('ChoicelistName', (45 * f,
     2 * f,
     550 * f,
     25 * f))
...
def CCcamConfigListEntry(file):
    ...
    if content == org:
        png = lock_on
        x, y, w, h = skin.parameters.get('SelectionListLock', (5 * f,
         0,
         25 * f,
         25 * f))
    else:
        png = lock_off
        x, y, w, h = skin.parameters.get('SelectionListLockOff', (5 * f,
         0,
         25 * f,
         25 * f))
    res.append(MultiContentEntryPixmapAlphaBlend(pos=(x, y), size=(w, h), png=png))
    x, y, w, h = skin.parameters.get('SelectionListDescr', (45 * f,
     2 * f,
     550 * f,
     25 * f))
    res.append(MultiContentEntryText(pos=(x, y), size=(w, h), font=0, text=name))
...
class CCcamInfoMain(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = NumberActionMap(['CCcamInfoActions'], {'1': self.keyNumberGlobal,
                ...
                if firstLine:
                    ...
                    if len(list) > 5:
                        ...
                        if caid != '':
                            ...
                            system = list[4].replace(' ', '')
                            infoList.append([_('CaID: ') + caid,
                             _('Provider: ') + provider,
                             _('Provider Name: ') + providername,
                             _('System: ') + system])
...
class CCcamInfoEcmInfoSelection(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': self.ok,
         'cancel': self.close}, -1)
...
class CCcamInfoInfoScreen(Screen):
    ...
    def __init__(self, session, info, set_title):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': self.close,
         'cancel': self.close,
         'up': self['text'].pageUp,
...
class CCcamShareViewMenu(Screen, HelpableScreen):
    ...
    def __init__(self, session, url):
        ...
        self['reshare'] = Label()
        self['title'] = Label()
        self['actions'] = HelpableNumberActionMap(self, 'CCcamInfoActions', {'cancel': (self.exit, _('close share view')),
    ...
    def readSharesCallback(self, html):
        ...
        self.instance.setTitle('%s (%s %d) %s %s' % (_('Share View'),
         _('Total cards:'),
         totalcards,
         _('Hops:'),
         ulevel))
        self['title'].setText('%s (%s %d) %s %s' % (_('Share View'),
         _('Total cards:'),
         totalcards,
         _('Hops:'),
         ulevel))
...
class CCcamInfoSubMenu(Screen):
    ...
    def __init__(self, session, list, infoList, set_title):
        ...
        self['list'] = MenuList(list)
        self['info'] = Label()
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': self.okClicked,
         'cancel': self.close}, -1)
...
class CCcamInfoConfigMenu(ConfigListScreen, Screen):
    ...
    def __init__(self, session, profile):
        ...
        ConfigListScreen.__init__(self, [getConfigListEntry(_('Name:'), config.cccaminfo.name),
...
class CCcamInfoRemoteBoxMenu(Screen):
    ...
    def __init__(self, session):
        ...
        self['list'] = MenuList([])
        self['actions'] = ActionMap(['CCcamInfoActions'], {'cancel': self.exit,
    ...
    def profileSelected(self):
        ...
        if len(self.list) > 0:
            ...
            if cur.ip == '':
                url = None
            elif cur.username != '' and cur.password != '':
                url = 'http://%s:%s@%s:%d' % (cur.username,
                 cur.password,
                 cur.ip,
                 cur.port)
            else:
...
class CCcamInfoShareInfo(Screen):
    ...
    def readSharesCallback(self, html):
        ...
        self.instance.setTitle('%s %d (%s%s / %s%s)' % (_('Available shares:'),
         count,
         _('Uphops: '),
         textUhops,
         _('Maxdown: '),
...
class CCcamInfoConfigSwitcher(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': self.activate,
         'cancel': self.close,
         'red': self.delete,
         'green': self.activate,
         'yellow': self.rename,
...
class CCcamInfoMenuConfig(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': self.changeState,
         'cancel': self.close,
         'red': self.close,
         'green': self.save,
