...
def InitCiConfig():
    ...
    if SystemInfo['CommonInterfaceCIDelay']:
        config.cimisc.dvbCiDelay = ConfigSelection(default='256', choices=[('16', _('16')),
         ('32', _('32')),
         ('64', _('64')),
         ('128', _('128')),
         ('256', _('256'))])
    ...
    if getBrandOEM() in ('entwopia', 'tripledot', 'dreambox'):
        if SystemInfo['HaveCISSL']:
            config.cimisc.civersion = ConfigSelection(default='ciplus1', choices=[('auto', _('Auto')),
             ('ciplus1', _('CI Plus 1.2')),
             ('ciplus2', _('CI Plus 1.3')),
             ('legacy', _('CI Legacy'))])
        else:
            config.cimisc.civersion = ConfigSelection(default='legacy', choices=[('legacy', _('CI Legacy'))])
    else:
        config.cimisc.civersion = ConfigSelection(default='auto', choices=[('auto', _('Auto')),
         ('ciplus1', _('CI Plus 1.2')),
         ('ciplus2', _('CI Plus 1.3')),
         ('legacy', _('CI Legacy'))])
...
class CIHelper(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'SetupActions'], {'ok': self.setupcihelper,
         'back': self.close,
...
class CIHelperSetup(Screen, ConfigListScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['WizardActions', 'ColorActions'], {'red': self.saveCIHelper,
         'back': self.close})
        self.updateList()
