    ...
    def __init__(self, session):
        ...
        if not self.paused_service:
            ...
            if self.timeHandler.ready():
                ...
                self.timeHandler = None
            elif config.misc.SyncTimeUsing.value == 0:
...
class QuitMainloopScreen(Screen):
    ...
    def __init__(self, session, retvalue = QUIT_SHUTDOWN):
        ...
        text = {QUIT_SHUTDOWN: _('Your %s %s is shutting down') % (getMachineBrand(), getMachineName()),
         ...
         QUIT_WOLSHUTDOWN: _('Your %s %s goes to WOL') % (getMachineBrand(), getMachineName())}.get(retvalue)
...
class TryQuitMainloop(MessageBox):
    ...
    def __init__(self, session, retvalue = QUIT_SHUTDOWN, timeout = -1, default_yes = True):
        ...
        if reason and inStandby:
            ...
            text = {QUIT_SHUTDOWN: _('Really shutdown now?'),
             ...
             QUIT_WOLSHUTDOWN: _('Really WOL now?')}.get(retvalue)
            if text:
