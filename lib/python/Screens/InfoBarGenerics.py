...
class InfoBarLongKeyDetection():
    ...
    def __init__(self):
        eActionMap.getInstance().bindAction('', -maxint - 1, self.detection)
...
class InfoBarUnhandledKey():
    ...
    def actionA(self, key, flag):
        try:
            print 'KEY: %s %s %s %s' % (key,
             flag,
             (key_name for key_name, value in KEYIDS.items() if value == key).next(),
             getKeyDescription(key)[0])
        except:
            try:
                print 'KEY: %s %s %s' % (key, flag, (key_name for key_name, value in KEYIDS.items() if value == key).next())
            except:
...
class InfoBarSimpleEventView():
    ...
    def __init__(self):
        self['EventViewActions'] = HelpableActionMap(self, 'InfobarEPGActions', {'showEventInfo': (self.openEventView, _('Show event details')),
...
class Seekbar(Screen):
    ...
    def __init__(self, session, fwd):
        ...
        self['time'] = Label()
        self['actions'] = ActionMap(['WizardActions', 'DirectionActions'], {'back': self.exit,
         'ok': self.keyOK,
         'left': self.keyLeft,
...
class InfoBarSeek():
    ...
    def seekFwd_new(self):
        ...
        if self.seekAction > 1 and self.seekAction <= int(config.seek.withjumps_after_ff_speed.getValue()):
            self.setSeekState(self.makeStateForward(self.seekAction))
        elif self.seekAction > int(config.seek.withjumps_after_ff_speed.getValue()):
            self.setSeekState(self.SEEK_STATE_PLAY)
...
class InfoBarExtensions():
    ...
    def __init__(self):
        ...
        if config.plisettings.ColouredButtons.value:
            self['InstantExtensionsActions'] = HelpableActionMap(self, 'InfobarExtensions', {'extensions': (self.bluekey_ex, _('Show extensions...')),
             ...
             'openDreamPlex': (self.showDreamPlex, _('Show the DreamPlex player...'))}, 1)
        else:
            self['InstantExtensionsActions'] = HelpableActionMap(self, 'InfobarExtensions', {'extensions': (self.bluekey_ex, _('view extensions...')),
             'showPluginBrowser': (self.showPluginBrowser, _('Show the plugin browser..')),
             'showDreamPlex': (self.showDreamPlex, _('Show the DreamPlex player...')),
             'showEventInfo': (self.SelectopenEventView, _('Show the infomation on current event.')),
             'showMediaPlayer': (self.showMediaPlayer, _('Show the media player...'))}, 1)
    ...
    def showAutoTimerList(self):
        if os.path.exists('/usr/lib/enigma2/python/Plugins/Extensions/AutoTimer/plugin.pyo'):
            ...
            self.session.openWithCallback(self.editCallback, AutoTimerOverview, self.autotimer)
        else:
...
class InfoBarInstantRecord():
    ...
    def instantRecord(self, serviceRef = None):
        ...
        if self.isInstantRecordRunning():
            ...
            list = ((_('Stop recording'), 'stop'),) + common + ((_('Change recording (duration)'), 'changeduration'), (_('Change recording (endtime)'), 'changeendtime'))
...
class InfoBarAudioSelection():
    ...
    def yellow_key(self):
        ...
        elif config.usage.on_long_yellow_key.value == '0':
            self.audioSelection()
...
class InfoBarSubserviceSelection():
    ...
    def __init__(self):
        self['SubserviceSelectionAction'] = HelpableActionMap(self, 'InfobarSubserviceSelectionActions', {'GreenPressed': self.GreenPressed,
    ...
    def GreenPressed(self):
        if config.plisettings.Subservice.value == '0':
            ...
            if serviceRef:
                ...
                elif config.plisettings.Subservice.value == '2':
                    self.openTimerList()
                else:
                    self.openPluginBrowser()
            elif config.plisettings.Subservice.value == '2':
...
class InfoBarRedButton():
    ...
    def __init__(self):
        ...
        self.onReadyForAIT = []
        self.__et = ServiceEventTracker(screen=self, eventmap={iPlayableService.evHBBTVInfo: self.detectedHbbtvApplication,
...
class InfoBarAspectSelection():
    ...
    def __init__(self):
        self['AspectSelectionAction'] = HelpableActionMap(self, 'InfobarAspectSelectionActions', {'aspectSelection': (self.ExGreen_toggleGreen, _('Aspect list...'))})
...
class InfoBarZoom():
    ...
    def __init__(self):
        self.zoomrate = 0
        self.zoomin = 1
        self['ZoomActions'] = HelpableActionMap(self, 'InfobarZoomActions', {'ZoomInOut': (self.ZoomInOut, _('Zoom In/Out TV...')),
         'ZoomOff': (self.ZoomOff, _('Zoom Off...'))}, prio=2)
...
class InfoBarHdmi():
    ...
    def __init__(self):
        ...
        self['HDMIActions'] = HelpableActionMap(self, 'InfobarHDMIActions', {'HDMIin': (self.HDMIIn, _('Switch to HDMI in mode')),
         'HDMIinLong': (self.HDMIInLong, _('Switch to HDMI in mode'))}, prio=2)
    ...
    def HDMIInPiP(self):
        if getMachineBuild() in ('dm7080', 'dm820', 'dm900', 'dm920'):
            ...
            if check.startswith('off'):
                ...
                f.write('off')
                f.close()
        elif not hasattr(self.session, 'pip') and not self.session.pipshown:
    ...
    def sleepTimerTimeout(self):
        if config.usage.sleep_timer_action.value != 'standby':
            ...
            if isRecordTime or isPowerTime:
                ...
                return
        if not Screens.Standby.inStandby:
            list = [(_('Yes'), True), (_('No'), False), (_('Extend'), 'extend')]
            if config.usage.sleep_timer_action.value == 'standby':
