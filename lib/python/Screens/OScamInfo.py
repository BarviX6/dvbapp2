...
class OscamInfo():
    ...
    def openWebIF(self, part = None, reader = None):
        ...
        if part is None:
            ...
            self.url = '%s://%s:%s/oscamapi.html?part=%s' % (self.proto,
             self.ip,
             self.port,
             part)
        if part is not None and reader is not None:
            self.url = '%s://%s:%s/oscamapi.html?part=%s&label=%s' % (self.proto,
             self.ip,
             self.port,
             part,
             reader)
            ...
            if not self.showLog:
                ...
                for cl in clients:
                    ...
                    if name != '' and name != 'anonymous' and proto != '':
                        try:
                            tmp[cl.attrib['type']].append((name,
                             proto,
                             '%s:%s' % (caid, srvid),
                             srvname_short,
                             ecmtime,
...
class OscamInfoMenu(Screen):
    ...
    def __init__(self, session):
        ...
        self.menu = [_('Show /tmp/ecm.info'),
         _('Show Clients'),
         _('Show Readers/Proxies'),
         _('Show Log'),
         _('Card infos (CCcam-Reader)'),
         _('ECM Statistics'),
         _('Setup')]
        Screen.__init__(self, session)
        self.osc = OscamInfo()
        self['mainmenu'] = oscMenuList([])
        self['actions'] = NumberActionMap(['OkCancelActions', 'InputActions', 'ColorActions'], {'ok': self.ok,
         'cancel': self.exit,
         'red': self.red,
         'green': self.green,
         'yellow': self.yellow,
        ...
        for t in mlist:
            ...
            if t.startswith('--'):
                ...
                if fileExists(png):
                    png = LoadPixmap(png)
                if png is not None:
                    x, y, w, h = skin.parameters.get('ChoicelistDash', (0,
                     2 * f,
                     800 * f,
                     2 * f))
...
class oscECMInfo(Screen, OscamInfo):
    ...
    def buildListEntry(self, listentry):
        return ['', (eListboxPythonMultiContent.TYPE_TEXT,
          ...
          0,
          RT_HALIGN_LEFT,
          listentry[0]), (eListboxPythonMultiContent.TYPE_TEXT,
          300 * f,
          2 * f,
          300 * f,
          30 * f,
...
class oscInfo(Screen, OscamInfo):
    ...
    def __init__(self, session, what):
        ...
        for k, v in enumerate(['red',
         'green',
         'yellow',
         'blue']):
            xpos = k * button_width
            self.skin += '<ePixmap name="%s" position="%d,%d" size="35,25" pixmap="/usr/share/enigma2/skin_default/buttons/key_%s.png" zPosition="1" transparent="1" alphatest="on" />' % (v,
             xpos,
             ypos,
             v)
            self.skin += '<widget source="key_%s" render="Label" position="%d,%d" size="%d,%d" font="Regular;18" zPosition="1" valign="center" transparent="1" />' % (v,
             xpos + 40,
             ypos,
             button_width,
             22)
        ...
        if config.oscaminfo.autoupdate.value:
            ...
            timeout = config.oscaminfo.intervall.value * 1000
            self.loop.start(timeout, False)
        self['actions'] = ActionMap(['OkCancelActions', 'ColorActions', 'DirectionActions'], {'ok': self.key_ok,
         'cancel': self.exit,
         'red': self.exit,
    ...
    def showData(self):
        ...
        if self.listchange:
            ...
            self['output'].instance.setScrollbarMode(0)
...
class oscEntitlements(Screen, OscamInfo):
    ...
    def __init__(self, session, reader):
        ...
        self['output'] = List([])
        self['actions'] = ActionMap(['OkCancelActions'], {'ok': self.showData,
         'cancel': self.exit}, -1)
        self.onLayoutFinish.append(self.showData)
