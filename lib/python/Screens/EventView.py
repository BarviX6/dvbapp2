...
class EventViewBase():
    ...
    def setEvent(self, event):
        ...
        nowt = time()
        now = localtime(nowt)
        begin = localtime(int(mktime((now.tm_year,
         int(begindate[1]),
         int(begindate[0]),
         int(begintime[0]),
         int(begintime[1]),
         ...
         now.tm_yday,
         now.tm_isdst))))
        end = localtime(int(mktime((now.tm_year,
         int(begindate[1]),
         int(begindate[0]),
         int(begintime[0]),
         int(begintime[1]),
         ...
         now.tm_wday,
         now.tm_yday,
         now.tm_isdst))) + event.getDuration())
...
class EventViewEPGSelect(Screen, EventViewBase):
    ...
    def __init__(self, session, event, ref, callback = None, singleEPGCB = None, multiEPGCB = None, similarEPGCB = None):
        ...
        self['yellow'] = Pixmap()
        self['blue'] = Pixmap()
        self['epgactions1'] = ActionMap(['OkCancelActions', 'EventViewActions'], {'timerAdd': self.timerAdd,
