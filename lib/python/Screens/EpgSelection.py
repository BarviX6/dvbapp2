...
class EPGSelection(Screen):
    ...
    def furtherOptions(self):
        ...
        if event:
            menu = [ (p.name, boundFunction(self.runPlugin, p)) for p in plugins.getPlugins(where=PluginDescriptor.WHERE_EVENTINFO) if 'selectedevent' in p.__call__.func_code.co_varnames ]
            if menu:
    ...
    def onSelectionChanged(self):
        ...
        if cur[1] is None or cur[1].getServiceName() == '':
            ...
            if self.key_red_choice != self.EMPTY:
                ...
                self.key_red_choice = self.EMPTY
            return
        if self.key_red_choice != self.ZAP and self.zapFunc is not None:
