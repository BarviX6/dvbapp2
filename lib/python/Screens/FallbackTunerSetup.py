...
class FallbackTunerSetup(ConfigListScreen, Screen):
    ...
    def __init__(self, session, menu_path = ''):
        ...
        self['actions2'] = ActionMap(['SetupActions'], {'ok': self.keyGo,
         'menu': self.keyCancel,
    ...
    def changedEntry(self):
        if self['config'].getCurrent() in (self.enabledEntry, self.addressTypeEntry):
            self.createSetup()
    ...
    def keyGo(self):
        ...
        if self.fallbackAddressType.value == 'domain':
            ...
            config.usage.remote_fallback.value = 'http://%d.%d.%d.%d:%d' % (self.ip.value[0],
             self.ip.value[1],
             self.ip.value[2],
             self.ip.value[3],
             self.port.value)
