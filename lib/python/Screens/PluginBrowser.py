...
class PluginBrowser(Screen, ProtectedScreen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['SetupActions', 'WizardActions'], {'ok': self.keyOk,
         'back': self.keyCancel,
         'menu': self.menu})
        self['PluginDownloadActions'] = ActionMap(['ColorActions'], {'red': self.delete,
         'green': self.download,
         'blue': self.keyBlue})
        self['SoftwareActions'] = ActionMap(['ColorActions'], {'red': self.keyRed,
         'green': self.keyGreen,
        ...
        self['MoveActions'] = ActionMap(['WizardActions'], {'left': self.keyLeft,
         ...
         'up': self.keyUp,
         'down': self.keyDown}, -1)
        self['NumberAction'] = NumberActionMap(['NumberActions'], {'0': self.resetSortOrder}, -1)
                ...
                if not plugin[0].endswith('-dev') and not plugin[0].endswith('-staticdev') and not plugin[0].endswith('-dbg') and not plugin[0].endswith('-doc') and not plugin[0].endswith('-src') and not plugin[0].endswith('-meta'):
                    for s in self.PLUGIN_PREFIX2:
                        if plugin[0].startswith(s):
                            ...
                            elif plugin[0] not in self.installedplugins:
                                if len(plugin) == 2:
...
class PluginFilter(ConfigListScreen, Screen):
    ...
    def __init__(self, session):
        ...
        self.createSetup()
        self['actions'] = ActionMap(['SetupActions', 'ColorActions', 'VirtualKeyboardActions'], {'ok': self.keySave,
