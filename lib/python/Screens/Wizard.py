...
class Wizard(Screen):
        ...
        def startElement(self, name, attrs):
            ...
            if name == 'step':
                ...
                if attrs.has_key('timeoutstep'):
                    ...
                    timeoutstep = ''
                self.wizard[self.lastStep] = {'id': id,
                 'condition': '',
                 'text': '',
                 'timeout': timeout,
    ...
    def __init__(self, session, showSteps = True, showStepSlider = True, showList = True, showConfig = True):
        ...
        self['actions'] = NumberActionMap(['WizardActions',
         ...
         'SetupActions',
         'InputAsciiActions',
         'KeyboardInputActions'], {'gotAsciiCode': self.keyGotAscii,
         'ok': self.ok,
         'back': self.back,
         'left': self.left,
         'right': self.right,
    ...
    def finished(self, gotoStep = None, *args, **kwargs):
        ...
        print_now = True
        if currStep == self.numSteps and self.wizard[currStep]['nextstep'] is None or self.wizard[currStep]['id'] == 'end':
            self.markDone()
            self.exit()
        else:
            self.codeafter = True
            ...
            if not self.runCode(self.wizard[currStep]['codeafter_async']):
                self.afterAsyncCode()
            elif self.updateValues in self.onShown:
    ...
    def updateValues(self):
        ...
        if not self.condition:
            if self.wizard[self.currStep].has_key('laststep'):
                self.markDone()
                self.exit()
                return
            self.currStep += 1
