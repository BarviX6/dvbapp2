...
class Tuner:
    ...
    def tuneTerr(self, frequency, inversion = 2, bandwidth = 7000000, fechigh = 6, feclow = 6, modulation = 2, transmission = 2, guard = 4, hierarchy = 4, system = 0, plp_id = 0):
        if self.frontend:
            print '[TuneTest] tuning to transponder with data', [frequency,
             inversion,
             bandwidth,
             fechigh,
             feclow,
...
class TuneTest:
    ...
    def updateStatus(self):
        ...
        if dict['tuner_state'] == 'TUNING':
            print 'TUNING'
            self.timer.start(100, True)
            self.progressCallback((self.getProgressLength(),
            ...
            if dict['tuner_state'] == 'LOSTLOCK' or dict['tuner_state'] == 'FAILED':
                ...
                self.failedTune.append([self.currTuned,
                 self.oldTuned,
                 'tune_failed',
                 dict])
                    ...
                    if self.currTuned is not None:
                        if self.tsid != self.currTuned[15] or self.onid != self.currTuned[16]:
                            self.failedTune.append([self.currTuned,
                             self.oldTuned,
                             'pids_failed',
                             {'real': (self.tsid, self.onid),
                              'expected': (self.currTuned[15], self.currTuned[16])},
                             dict])
                            pidsFailed = True
                        else:
                            self.successfullyTune.append([self.currTuned, self.oldTuned, dict])
    ...
    def getProgressLength(self):
        ...
        elif count < self.stopOnError:
            count = self.stopOnError
        if self.stopOnSuccess == -1:
            count = len(self.transponderlist)
        elif count < self.stopOnSuccess:
