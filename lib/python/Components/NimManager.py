...
from os import path
from enigma import eDVBSatelliteEquipmentControl as secClass, eDVBSatelliteLNBParameters as lnbParam, eDVBSatelliteDiseqcParameters as diseqcParam, eDVBSatelliteSwitchParameters as switchParam, eDVBSatelliteRotorParameters as rotorParam, eDVBResourceManager, eDVBDB, eEnv, iDVBFrontend, pNavigation
...
iDVBFrontendDict = {iDVBFrontend.feSatellite: 'DVB-S',
...
class SecConfigure():
    ...
    def update(self):
        ...
        try:
            for slot in nim_slots:
                if slot.frontend_id is not None:
                    types = [ type for type in ['DVB-C',
                     'DVB-T',
                     'DVB-T2',
                     'DVB-S',
                     'DVB-S2',
                     'ATSC'] if eDVBResourceManager.getInstance().frontendIsCompatible(slot.frontend_id, type) ]
                    if 'DVB-T2' in types:
                        types.remove('DVB-T')
                    if 'DVB-S2' in types:
                        types.remove('DVB-S')
        ...
        for slot in nim_slots:
            if slot.type is not None:
                used_nim_slots.append((slot.slot,
                 slot.description,
                 slot.canBeCompatible('ATSC') and (slot.config.atsc.configMode.value != 'nothing' and True or False) or slot.canBeCompatible('DVB-S') and (slot.config.dvbs.configMode.value != 'nothing' and True or False) or slot.canBeCompatible('DVB-C') and (slot.config.dvbc.configMode.value != 'nothing' and True or False) or slot.canBeCompatible('DVB-T') and (slot.config.dvbt.configMode.value != 'nothing' and True or False) or slot.canBeCompatible('DVB-S2') and (slot.config.dvbs.configMode.value != 'nothing' and True or False),
        ...
        for slot in nim_slots:
            ...
            if slot.canBeCompatible('DVB-S'):
                ...
                if nim.configMode.value in ('loopthrough', 'satposdepends', 'nothing'):
                    ...
                    if nim.configMode.value == 'equal':
                        pass
                    elif nim.configMode.value == 'simple':
                        print 'diseqcmode: ', nim.diseqcMode.value
                        if nim.diseqcMode.value == 'single':
                        ...
                        elif nim.diseqcMode.value == 'toneburst_a_b':
                        ...
                        elif nim.diseqcMode.value == 'diseqc_a_b':
                        ...
                        elif nim.diseqcMode.value == 'diseqc_a_b_c_d':
                            fastDiSEqC = nim.simpleDiSEqCOnlyOnSatChange.value
                            setVoltageTone = nim.simpleDiSEqCSetVoltageTone.value
                            self.addLNBSimple(sec, slotid=x, orbpos=nim.diseqcA.orbital_position, toneburstmode=diseqcParam.NO, diseqcmode=diseqcParam.V1_0, diseqcpos=diseqcParam.AA, fastDiSEqC=fastDiSEqC, setVoltageTone=setVoltageTone, diseqc13V=nim.diseqc13V.value)
                            self.addLNBSimple(sec, slotid=x, orbpos=nim.diseqcB.orbital_position, toneburstmode=diseqcParam.NO, diseqcmode=diseqcParam.V1_0, diseqcpos=diseqcParam.AB, fastDiSEqC=fastDiSEqC, setVoltageTone=setVoltageTone, diseqc13V=nim.diseqc13V.value)
                        ...
                        elif nim.diseqcMode.value in ('positioner', 'positioner_select'):
                            current_mode = 3
                            sat = 0
                            ...
                            if nim.powerMeasurement.value:
                                useInputPower = True
                                turn_speed_dict = {'fast': rotorParam.FAST,
                                 'slow': rotorParam.SLOW}
                            ...
                            self.addLNBSimple(sec, slotid=x, diseqcmode=current_mode, orbpos=sat, longitude=nim.longitude.float, loDirection=loValue, latitude=nim.latitude.float, laDirection=laValue, turningSpeed=turning_speed, useInputPower=useInputPower, inputPowerDelta=inputPowerDelta, diseqc13V=nim.diseqc13V.value)
                    elif nim.configMode.value == 'advanced':
        ...
        for x in range(1, 71):
            if len(lnbSat[x]) > 0:
                ...
                if x <= maxFixedLnbPositions:
                    posnum = x
                    sec.setLNBSatCRpositionnumber(x)
                else:
                    sec.setLNBSatCRpositionnumber(0)
                tunermask = 1 << slotid
                if slotid in self.equal:
                    for slot in self.equal[slotid]:
                        tunermask |= 1 << slot
                ...
                if currLnb.lof.value == 'universal_lnb':
                    ...
                    def setupUnicable(configManufacturer, ProductDict):
                        ...
                        if product_name in manufacturer_scr:
                            ...
                            if diction != 'EN50607' or posnum <= positionsoffset + manufacturer_positions_value and posnum > positionsoffset and x <= maxFixedLnbPositions:
                                ...
                                sec.setLNBSatCRTuningAlgo(['traditional',
                                 'reliable',
                                 'traditional_retune',
                                 'reliable_retune'].index(currLnb.unicableTuningAlgo.value))
                ...
                if dm != 'none':
                    ...
                    if currLnb.diseqcMode.value == '1_0':
                        ...
                        if udc > 0:
                            sec.setUncommittedCommand(240 | udc - 1)
                        else:
                            sec.setUncommittedCommand(0)
                        sec.setRepeats({'none': 0,
                         'one': 1,
                         'two': 2,
                         'three': 3}[currLnb.diseqcRepeats.value])
    ...
    def reconstructUnicableDate(self, configManufacturer, ProductDict, currLnb):
        ...
        PDict = SDict.get(ManufacturerName, None)
        if PDict is None:
            return
        PN = PDict.get('product', None)
        if PN is None:
            return
        if ManufacturerName in ProductDict.keys():
        ...
        if PN not in tmp.product.choices.choices:
            ...
            positionslist = [1, (9750, 10600, 11700)]
            ...
            for cnt in range(0, SatCR + 1):
                vcofreq = cnt == SatCR and vco or 0
                if vcofreq == 0:
            ...
            dictionlist = [('EN50494', 'Unicable(EN50494)')]
        ...
        if ManufacturerName not in configManufacturer.choices.choices:
            configManufacturer.choices.choices.append(ManufacturerName)
...
class NIM(object):
    ...
    def connectableTo(self):
        connectable = {'DVB-S': ('DVB-S', 'DVB-S2'),
         'DVB-C': ('DVB-C', 'DVB-C2'),
         'DVB-T': ('DVB-T', 'DVB-T2'),
         'DVB-S2': ('DVB-S', 'DVB-S2'),
        ...
        if self.hasNimType('DVB-S'):
            ...
            if db.readSatellites(self.satList, self.satellites, self.transponders):
                ...
                def emergencyAid():
                    ...
                    t2_sv3 = ('frequency', 'symbol_rate', 'polarization', 'fec_inner', 'position', 'inversion', 'system', 'modulation', 'rolloff', 'pilot')
                    t2_sv4 = ('frequency', 'symbol_rate', 'polarization', 'fec_inner', 'position', 'inversion', 'flags', 'system', 'modulation', 'rolloff', 'pilot')
                    tplist = []
                    for x in transponders:
                        ...
                        if freq[0] == 's' or freq[0] == 'S':
                            ...
                            if int(tp.get('namespace'), 16) >> 16 & 4095 != int(tp.get('position')):
                                print 'Namespace %s and Position %s are not identical' % (tp.get('namespace'), tp.get('position'))
                                continue
                            if version >= 4:
                                tp.update({'supposition': int(tp.get('namespace', '0'), 16) >> 24 & 15})
...
def InitNimManager(nimmgr, update_slots = None):
    ...
    for manufacturer in entry.getchildren():
        ...
        for product in manufacturer.getchildren():
            ...
            p_update({'positions': tuple(positions)})
            bootuptime = product.get('bootuptime', 2700)
            p_update({'bootuptime': tuple([bootuptime])})
            m_update({product.get('name'): p})
    ...
    unicable_choices = {'unicable_lnb': _('Unicable LNB'),
    ...
    diseqc_mode_choices = [('single', _('Single')),
    ...
    advanced_satlist_choices = nimmgr.satList + [(3601, _('All satellites 1 (USALS)'), 1),
    ...
    advanced_lnb_commandOrder_choices = [('ct', 'DiSEqC 1.0, toneburst'),
        ...
        if configElement.value == 'unicable':
            ...
            if isinstance(section.unicable, ConfigNothing):
                if lnb == 1 or lnb > maxFixedLnbPositions:
                    ...
                    section.unicable = ConfigSelection(choices={'unicable_matrix': _('Unicable Matrix'),
                     'unicable_user': 'Unicable ' + _('User defined')}, default='unicable_matrix')
            ...
            section.unicableTuningAlgo = ConfigSelection([('reliable', _('reliable')),
             ('traditional', _('traditional (fast)')),
             ('reliable_retune', _('reliable, retune')),
             ('traditional_retune', _('traditional (fast), retune'))], default='reliable_retune')
    ...
    def createSatConfig(nim, x, empty_slots):
        try:
            ...
            nim.toneAmplitude = ConfigSelection([('11', '340mV'),
             ('10', '360mV'),
             ('9', '600mV'),
             ('8', '700mV'),
             ('7', '800mV'),
    ...
    for slot in nimmgr.nim_slots:
        ...
        if slot.canBeCompatible('DVB-S'):
            ...
            config_mode_choices = [('nothing', _('nothing connected')), ('simple', _('simple')), ('advanced', _('advanced'))]
        ...
        if slot.canBeCompatible('DVB-C'):
            ...
            nim.configMode = ConfigSelection(choices={'enabled': _('enabled'),
             'nothing': _('nothing connected')}, default=default)
    ...
    for slot in nimmgr.nim_slots:
        ...
        print '[NimManager] slotname = %s, slotdescription = %s, multitype = %s, current type = %s' % (slot.input_name,
         slot.description,
         slot.isMultiType(),
         slot.getType())
