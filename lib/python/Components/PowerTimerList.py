from enigma import eListboxPythonMultiContent, eListbox, gFont, getDesktop, RT_HALIGN_LEFT, RT_HALIGN_RIGHT, RT_VALIGN_TOP, RT_VALIGN_BOTTOM
...
def gettimerType(timer):
    timertype = {TIMERTYPE.NONE: _('Nothing'),
     ...
     TIMERTYPE.RESTART: _('Restart GUI')}[timer.timerType]
    return timertype


def getafterEvent(timer):
    afterevent = {AFTEREVENT.NONE: _('Nothing'),
     AFTEREVENT.WAKEUP: _('Wake Up'),
     AFTEREVENT.WAKEUPTOSTANDBY: _('Wake Up To Standby'),
     AFTEREVENT.STANDBY: _('Standby'),
     AFTEREVENT.DEEPSTANDBY: _('Deep Standby')}[timer.afterEvent]
    return afterevent
    ...
    def buildTimerEntry(self, timer, processed):
        ...
        if timer.timerType == TIMERTYPE.AUTOSTANDBY or timer.timerType == TIMERTYPE.AUTODEEPSTANDBY:
            ...
            res.append((eListboxPythonMultiContent.TYPE_TEXT,
             self.satPosLeft,
             self.rowSplit,
             width - self.satPosLeft,
             self.itemHeight - self.rowSplit,
             1,
             RT_HALIGN_RIGHT | RT_VALIGN_TOP,
             autosleepwindow + _('Delay:') + ' ' + str(timer.autosleepdelay) + ' (' + _('mins') + ')'))
        else:
            res.append((eListboxPythonMultiContent.TYPE_TEXT,
             self.satPosLeft,
             0,
