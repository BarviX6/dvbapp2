...
class InfoBarTimeshift:
    ...
    def __init__(self):
        self['TimeshiftActions'] = HelpableActionMap(self, 'InfobarTimeshiftActions', {'timeshiftStart': (self.startTimeshift, _('Start timeshift')),
         ...
         'seekdef:9': (boundFunction(self.seekdef, 9), _('Seek'))}, prio=1)
        ...
        if savefilename is None:
            ...
            try:
                if timeshiftfile is None:
                    ...
                    try:
                        if config.usage.setup_level.index >= 2:
                            if config.recording.filename_composition.value == 'long' and self.pts_curevent_name.replace('\n', '') != self.pts_curevent_description.replace('\n', ''):
                                ptsfilename = '%s - %s - %s - %s' % (strftime('%Y%m%d %H%M', localtime(self.pts_starttime)),
                                 self.pts_curevent_station,
                                 self.pts_curevent_name.replace('\n', ''),
                                 self.pts_curevent_description.replace('\n', ''))
                    ...
                    metafile.write('%s\n%s\n%s\n%i\n%s' % (self.pts_curevent_servicerefname,
                     self.pts_curevent_name.replace('\n', ''),
                     self.pts_curevent_description.replace('\n', ''),
                     int(self.pts_starttime),
                     metamergestring))
                    ...
                    try:
                        if config.usage.setup_level.index >= 2:
                            if config.recording.filename_composition.value == 'long' and eventname != description:
                                ptsfilename = '%s - %s - %s - %s' % (strftime('%Y%m%d %H%M', localtime(int(begintime))),
                                 servicename,
                                 eventname,
                                 description)
                            elif config.recording.filename_composition.value == 'short':
    ...
    def ptsAskUser(self, what):
        ...
        choice_restart = [(_('Delete the current timeshift buffer and restart timeshift'), 'restarttimeshift'), (_('Nothing, just leave this menu'), 'no')]
        choice_save = [(_('Stop timeshift and save timeshift buffer as movie and start recording of current event'), 'savetimeshiftandrecord'),
         ...
         (_('Nothing, just leave this menu'), 'no')]
        choice_livetv = [(_('No'), 'nolivetv'), (_('Yes'), 'golivetv')]
        if what == 'time':
            ...
            if filename.endswith('.meta'):
                ...
                if ptsgetnextfile:
                    ...
                    if ASCIItranslit.legacyEncode(eventname) == ASCIItranslit.legacyEncode(ptsmergeeventname):
                        ...
                        JobManager.AddJob(MergeTimeshiftJob(self, 'cat "%s%s" >> "%s%s"' % (config.usage.autorecord_path.value,
                         ptsmergeSRC,
                         config.usage.autorecord_path.value,
                         ptsmergeDEST), ptsmergeSRC, ptsmergeDEST, eventname))
                        config.timeshift.isRecording.value = True
                        ptsfilemerged = True
                    else:
                        ptsgetnextfile = True
