...
class ConfigSelection(ConfigElement):
    ...
    def getMulti(self, selected):
        ...
        if self._descr is None:
            self._descr = self.description[self.value]
        keywords_true = (_('True'),
         _('Yes'),
         _('Enabled'),
         _('On'))
        keywords_false = (_('False'),
         _('No'),
         _('Disable'),
         _('Disabled'),
         _('Off'),
...
class ConfigBoolean(ConfigElement):
    ...
    def __init__(self, default = False, descriptions = {False: _('false'),
 True: _('true')}, graphic = True):
        ConfigElement.__init__(self)
...
class ConfigYesNo(ConfigBoolean):
    ...
    def __init__(self, default = False):
        ConfigBoolean.__init__(self, default=default, descriptions={False: _('no'),
         True: _('yes')})
...
class ConfigOnOff(ConfigBoolean):
    ...
    def __init__(self, default = False):
        ConfigBoolean.__init__(self, default=default, descriptions={False: _('off'),
         True: _('on')})
...
class ConfigEnableDisable(ConfigBoolean):
    ...
    def __init__(self, default = False):
        ConfigBoolean.__init__(self, default=default, descriptions={False: _('disable'),
         True: _('enable')})
    ...
    def handleKey(self, key):
        ...
        if key == KEY_DELETE and self.wideformat:
            ...
            if self.wideformat:
                ...
                if hour == 0:
                    hour = 12
                if self.marked_pos == 0 and digit >= 2:
                    return
                if self.marked_pos == 1 and hour > 9 and digit >= 3:
                    return
                if self.marked_pos == 1 and hour < 10 and digit == 0:
                    return
            else:
                if self.marked_pos == 0 and digit >= 3:
                    return
                if self.marked_pos == 1 and hour > 19 and digit >= 4:
                    return
            if self.marked_pos == 2 and digit >= 6:
                return
            value = bytearray('%02d%02d' % (hour, self._value[1]))
            value[self.marked_pos] = digit + ord('0')
            hour = int(value[:2])
            minute = int(value[2:])
            if self.wideformat:
                if hour == 12:
