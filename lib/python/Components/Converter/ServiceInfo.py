...
class ServiceInfo(Poll, Converter, object):
    ...
    def __init__(self, type):
        ...
        self.type, self.interesting_events = {'HasTelext': (self.HAS_TELETEXT, (iPlayableService.evUpdatedInfo,)),
         ...
         'VideoInfo': (self.VIDEO_INFO, (iPlayableService.evVideoSizeChanged,
         ...
         'IsHDHDR': (self.IS_HDHDR, (iPlayableService.evVideoSizeChanged, iPlayableService.evVideoGammaChanged))}[type]
    ...
    def getValue(self):
        ...
        if self.type == self.VIDEO_PARAMS:
            if info.getInfo(iServiceInformation.sVideoHeight) < 0 or info.getInfo(iServiceInformation.sFrameRate) < 0 or info.getInfo(iServiceInformation.sProgressive) < 0:
                return -1
            return -2
        return -1
