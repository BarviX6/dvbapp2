    ...
    def getText(self):
        ...
        if self.type < self.TYPE_VFD_LENGTH:
            if config.usage.elapsed_time_positive_osd.value:
                ...
                sign_r = '+'
                sign_l = ''
            if config.usage.swap_media_time_display_on_osd.value == '1':
                ...
                if self.type == self.TYPE_POSITION:
                    ...
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                        return sign_p + ngettext('%d Min', '%d Mins', p / 60) % (p / 60)
                    elif config.usage.swap_time_remaining_on_osd.value == '2' or config.usage.swap_time_remaining_on_osd.value == '3':
                        return ''
                    else:
                        return sign_r + ngettext('%d Min', '%d Mins', r / 60) % (r / 60)
            elif config.usage.swap_media_time_display_on_osd.value == '2':
                if self.type == self.TYPE_LENGTH:
                ...
                if self.type == self.TYPE_POSITION:
                    ...
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                        ...
                        return sign_r + '%d:%02d' % (r / 60, r % 60)
            elif config.usage.swap_media_time_display_on_osd.value == '3':
                if self.type == self.TYPE_LENGTH:
                ...
                if self.type == self.TYPE_POSITION:
                    ...
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                        ...
                        return sign_r + '%d:%02d' % (r / 3600, r % 3600 / 60)
            elif config.usage.swap_media_time_display_on_osd.value == '4':
                if self.type == self.TYPE_LENGTH:
                ...
                if self.type == self.TYPE_POSITION:
                    ...
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                        ...
                        return sign_r + '%d:%02d:%02d' % (r / 3600, r % 3600 / 60, r % 60)
            elif config.usage.swap_media_time_display_on_osd.value == '5':
                if self.type == self.TYPE_LENGTH:
                    return sign_l + '%d:%02d' % (l / 3600, l % 3600 / 60)
                if self.type == self.TYPE_POSITION:
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                    ...
                    if config.usage.swap_time_remaining_on_osd.value == '1':
                        ...
                        if config.usage.swap_time_remaining_on_osd.value == '2' or config.usage.swap_time_remaining_on_osd.value == '3':
                            return ''
                        try:
                            return sign_r + '%d%%' % (float(p + 0.0) / float(l + 0.0) * 100)
                        except:
                ...
                if not self.vfd:
                    if self.showHours:
                        ...
                        if self.type == self.TYPE_POSITION:
                            return sign_p + ngettext('%d Min', '%d Mins', p / 60) % (p / 60)
                        if self.type == self.TYPE_REMAINING and self.OnlyMinute:
                ...
                if self.type == self.TYPE_LENGTH:
                    return sign_l + '%d:%02d:%02d:%03d' % (l / 3600 / 90000,
                     l / 90000 % 3600 / 60,
                     l / 90000 % 60,
                     l % 90000 / 90)
                if self.type == self.TYPE_POSITION:
                    return sign_r + '%d:%02d:%02d:%03d' % (r / 3600 / 90000,
                     r / 90000 % 3600 / 60,
                     r / 90000 % 60,
                     r % 90000 / 90)
                if self.type == self.TYPE_REMAINING:
                    return sign_p + '%d:%02d:%02d:%03d' % (p / 3600 / 90000,
                     p / 90000 % 3600 / 60,
                     p / 90000 % 60,
                     p % 90000 / 90)
            else:
            ...
            if config.usage.elapsed_time_positive_vfd.value:
                ...
                sign_p = '-'
                sign_r = '+'
            if config.usage.swap_media_time_display_on_vfd.value == '1':
                if self.type == self.TYPE_VFD_LENGTH:
                    return ngettext('%d Min', '%d Mins', l / 60) % (l / 60)
                if self.type == self.TYPE_VFD_POSITION:
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        return sign_p + ngettext('%d Min', '%d Mins', p / 60) % (p / 60)
                    ...
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        return sign_p + ngettext('%d Min', '%d Mins', p / 60) % (p / 60)
                    elif config.usage.swap_time_remaining_on_vfd.value == '2' or config.usage.swap_time_remaining_on_vfd.value == '3':
                        return ''
                    else:
                        return sign_r + ngettext('%d Min', '%d Mins', r / 60) % (r / 60)
            elif config.usage.swap_media_time_display_on_vfd.value == '2':
                if self.type == self.TYPE_VFD_LENGTH:
                    return sign_l + '%d:%02d' % (l / 60, l % 60)
                if self.type == self.TYPE_VFD_POSITION:
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                    ...
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        ...
                        return sign_r + '%d:%02d' % (r / 60, r % 60)
            elif config.usage.swap_media_time_display_on_vfd.value == '3':
                if self.type == self.TYPE_VFD_LENGTH:
                    return sign_l + '%d:%02d' % (l / 3600, l % 3600 / 60)
                if self.type == self.TYPE_VFD_POSITION:
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                    ...
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        ...
                        return sign_r + '%d:%02d' % (r / 3600, r % 3600 / 60)
            elif config.usage.swap_media_time_display_on_vfd.value == '4':
                if self.type == self.TYPE_VFD_LENGTH:
                    return sign_l + '%d:%02d:%02d' % (l / 3600, l % 3600 / 60, l % 60)
                if self.type == self.TYPE_VFD_POSITION:
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                    ...
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        ...
                        return sign_r + '%d:%02d:%02d' % (r / 3600, r % 3600 / 60, r % 60)
            elif config.usage.swap_media_time_display_on_vfd.value == '5':
                if self.type == self.TYPE_VFD_LENGTH:
                    return sign_l + '%d:%02d' % (l / 3600, l % 3600 / 60)
                if self.type == self.TYPE_VFD_POSITION:
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                    ...
                    if config.usage.swap_time_remaining_on_vfd.value == '1':
                        ...
                        if config.usage.swap_time_remaining_on_vfd.value == '2' or config.usage.swap_time_remaining_on_vfd.value == '3':
                            return ''
                        try:
                            return sign_r + '%d%%' % (float(p + 0.0) / float(l + 0.0) * 100)
                        except:
                ...
                if self.showHours:
                    ...
                    if self.type == self.TYPE_VFD_POSITION:
                        return sign_p + ngettext('%d Min', '%d Mins', p / 60) % (p / 60)
                    if self.type == self.TYPE_VFD_REMAINING:
                        return sign_r + ngettext('%d Min', '%d Mins', r / 60) % (r / 60)
                else:
                    if self.type == self.TYPE_VFD_LENGTH:
                        return sign_l + '%d:%02d' % (l / 60, l % 60)
