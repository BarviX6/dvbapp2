    ...
    def autoDimUpLCD(self):
        if not Screens.Standby.inTryQuitMainloop:
            ...
            if self.currBrightness < self.Brightness:
                ...
                eDBoxLCD.getInstance().setLCDBrightness(self.currBrightness)
                self.autoDimUpLCDTimer.start(10, True)
            elif self.dimBrightness is not None and self.currBrightness > self.dimBrightness and self.dimDelay is not None and self.dimDelay > 0:
...
def InitLcd():
    ...
    if detected:
        ...
        if can_lcdmodechecking:
            ...
            config.lcd.modepip = ConfigSelection(choices={'0': _('off'),
             '5': _('PIP'),
             '7': _('PIP with OSD')}, default='0')
            config.lcd.modepip.addNotifier(setLCDModePiP)
            config.lcd.screenshot = ConfigYesNo(default=False)
            config.lcd.screenshot.addNotifier(setLCDScreenshot)
            config.lcd.modeminitv = ConfigSelection(choices={'0': _('normal'),
             '1': _('MiniTV'),
             '2': _('OSD'),
             '3': _('MiniTV with OSD')}, default='0')
        ...
        config.lcd.scroll_speed = ConfigSelection(default='300', choices=[('500', _('slow')), ('300', _('normal')), ('100', _('fast'))])
        config.lcd.scroll_delay = ConfigSelection(default='10000', choices=[('10000', '10 ' + _('seconds')),
         ('20000', '20 ' + _('seconds')),
         ('30000', '30 ' + _('seconds')),
         ('60000', '1 ' + _('minute')),
         ('300000', '5 ' + _('minutes')),
        ...
        config.usage.lcd_ledpowercolor = ConfigSelection(default='1', choices=[('0', _('off')),
         ('1', _('blue')),
         ('2', _('red')),
         ('3', _('violet'))])
        config.usage.lcd_ledpowercolor.addNotifier(setLedPowerColor)
        config.usage.lcd_ledstandbycolor = ConfigSelection(default='3', choices=[('0', _('off')),
         ('1', _('blue')),
         ('2', _('red')),
         ('3', _('violet'))])
        config.usage.lcd_ledstandbycolor.addNotifier(setLedStandbyColor)
        config.usage.lcd_ledsuspendcolor = ConfigSelection(default='2', choices=[('0', _('off')),
         ('1', _('blue')),
         ('2', _('red')),
         ('3', _('violet'))])
        ...
        if getBoxType() in ('formuler1', 'osminiplus'):
            config.usage.lcd_dateformat = ConfigSelection(default='%H:%M', choices=[('OFF', 'Off'),
            ...
            config.usage.lcd_dateformat = ConfigSelection(default='%H:%M %d/%m', choices=[('OFF', 'Off'),
             ('%H:%M', str(ntime.strftime(_('%H:%M')))),
            ...
            config.usage.lcd_dateformat = ConfigSelection(default='%H:%M %d/%m/%Y', choices=[('OFF', 'Off'),
             ('%H:%M', str(ntime.strftime(_('%H:%M')))),
             ('%H:%M %d/%m', str(ntime.strftime(_('%H:%M %d/%m')))),
        ...
        config.lcd.dimdelay = ConfigSelection(default='0', choices=[('5', '5 ' + _('seconds')),
         ('10', '10 ' + _('seconds')),
         ('15', '15 ' + _('seconds')),
         ('20', '20 ' + _('seconds')),
         ('30', '30 ' + _('seconds')),
        ...
        if SystemInfo['LCDMiniTV'] and config.misc.boxtype.value not in ('gbquad', 'gbquadplus', 'gbquad4k', 'gbue4k'):
            config.lcd.minitvmode = ConfigSelection([('0', _('normal')),
             ('1', _('MiniTV')),
             ('2', _('OSD')),
